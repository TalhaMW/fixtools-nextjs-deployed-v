...
const {
  ...
  CssMinifi,
  CssBeautify,
} = require('../controller/CssTools.js');
const {
  XmlToJson,
  JsonToYaml,
  JsonToBase64,
  ...
  TsvToJson,
  JsonToTsv,
} = require('../controller/JsonTool');
const { HtmlMinfi } = require('../controller/HtmlTool');

routers.post('/movieEmoji', textToEmoji);
routers.post('/qa', questionAnswer);
routers.post('/correction', grammerCorrection);

routers.post('/summarize', summerizeContent);
...
routers.post('/lan', languageTranslator);
